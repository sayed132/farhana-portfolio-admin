...
const Education = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (res.data.success) {
      ...
      const portfolioRes = await axiosPublic.post("/edu/create", education);
  ...
  const handleDelete = (product) => {
    fetch(`https://farhans-s-server.onrender.com/edu/delete/${product}`, {
      method: "DELETE",
      // headers: {
        ...
        <dialog id="update_modal" className="modal" open>
          ...
          >
            <div className="flex justify-between items-start">
              <h1 className="pb-8 text-lg backdrop-blur-sm">Update Edu Info</h1>
              <p
                className="font-semibold cursor-pointer"
                onClick={() => setEduUpdateModal(false)}
              >
                X
              </p>
            </div>
